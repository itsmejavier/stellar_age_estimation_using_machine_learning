...
X_MSS_train = scaler.fit_transform(X_MSS_train)
X_MSS_test = scaler.transform(X_MSS_test)

# Membangun model ANN dengan Keras
model_MSS = Sequential()
model_MSS.add(Dense(64, input_dim=X_MSS_train.shape[1], activation='relu'))
model_MSS.add(Dense(32, activation='relu'))
model_MSS.add(Dense(16, activation='relu'))
model_MSS.add(Dense(1))  # Output layer

# Compile model
model_MSS.compile(optimizer='adam', loss='mean_squared_error')

# Melatih model
history_MSS = model_MSS.fit(X_MSS_train,y_MSS_train,epochs=500,batch_size=10,verbose=1,validation_split=0.2)

# Melakukan prediksi pada data pengujian
y_MSS_pred = model_MSS.predict(X_MSS_test)

# Pastikan y_MSS_pred dan y_MSS_test berdimensi 1
y_MSS_pred = y_MSS_pred.ravel()
y_MSS_test = y_MSS_test.values.ravel()
...
# Normalisasi data
scaler = StandardScaler()
X_RGS_train = scaler.fit_transform(X_RGS_train)
X_RGS_test = scaler.transform(X_RGS_test)

# Membangun model ANN dengan Keras
model_RGS = Sequential()
model_RGS.add(Dense(64, input_dim=X_RGS_train.shape[1], activation='relu'))
model_RGS.add(Dense(32, activation='relu'))
model_RGS.add(Dense(16, activation='relu'))
model_RGS.add(Dense(1))  # Output layer

# Compile model
model_RGS.compile(optimizer='adam', loss='mean_squared_error')

# Melatih model
history_RGS = model_RGS.fit(X_RGS_train,y_RGS_train,epochs=500,batch_size=10,verbose=1,validation_split=0.2)

# Melakukan prediksi pada data pengujian
y_RGS_pred = model_RGS.predict(X_RGS_test)

# Pastikan y_RGS_pred dan y_RGS_test berdimensi 1
y_RGS_pred = y_RGS_pred.ravel()
y_RGS_test = y_RGS_test.values.ravel()
...
plt.title("Scatter Plot Umur Bintang\n Menggunakan ANN") # Beri judul scatter plot.
...
# Gunakan fungsi open().
modelMSS = 'C:\\Users\\ACER\\Downloads\\Tugas Akhir Astronomi\\Uji Coba ANN\\Version 0\\modelMSS.txt'

# Buka file di write mode
with open(modelMSS,'w') as file:
  # Write content to the file
  file.write(str(model_MSS))

    
# Simpan model untuk Bintang Raksasa Merah.
# Gunakan fungsi open().
modelRGS = 'C:\\Users\\ACER\\Downloads\\Tugas Akhir Astronomi\\Uji Coba ANN\\Version 0\\modelRGS.txt'

# Buka file di write mode
with open(modelRGS,'w') as file:
  # Write content to the file
  file.write(str(model_RGS))
...
# Simpan data.
df_y_MSS_diff_table.to_csv('C:\\Users\\ACER\\Downloads\\Tugas Akhir Astronomi\\Uji Coba ANN\\Version 0\\MainSequenceStars.csv')
df_y_RGS_diff_table.to_csv('C:\\Users\\ACER\\Downloads\\Tugas Akhir Astronomi\\Uji Coba ANN\\Version 0\\RedGiantStars.csv')
...
# Simpan data.
df_metric_MSS.to_csv('C:\\Users\\ACER\\Downloads\\Tugas Akhir Astronomi\\Uji Coba ANN\\Version 0\\Metric_MainSequenceStars.csv')
df_metric_RGS.to_csv('C:\\Users\\ACER\\Downloads\\Tugas Akhir Astronomi\\Uji Coba ANN\\Version 0\\Metric_RedGiantStars.csv')
