...
catboost_model_MSS = CatBoostRegressor(metric_period=1000)

# Buat rentang learning rate yang akan diuji
lr_arr = np.arange(0.01, 1.03, 0.03)
lr = (list(lr_arr))
for i in range(len(lr)):
    lr[i] = round(lr[i], 2)

# Buat rentang iterations yang akan diuji
iters_arr = np.arange(25, 1025, 25)
iters = list(iters_arr)

# Set hyperparameter yang akan diuji
param_grid = {
    'iterations': iters,     # Jumlah iterasi
    'learning_rate': lr      # Tingkat pembelajaran
}

# Gunakan GridSearchCV untuk mencari kombinasi hyperparameter terbaik
grid_search_MSS = GridSearchCV(catboost_model_MSS,param_grid,cv=5,scoring='neg_mean_squared_error')
...
best_catboost_model_MSS = CatBoostRegressor(**best_params_MSS)
best_catboost_model_MSS.fit(X_MSS_train, y_MSS_train)

# Lakukan prediksi pada data pengujian
y_MSS_pred = best_catboost_model_MSS.predict(X_MSS_test)
...
# Definisi model CatBoost
catboost_model_RGS = CatBoostRegressor(metric_period=1000)
...
# Gunakan GridSearchCV untuk mencari kombinasi hyperparameter terbaik
grid_search_RGS = GridSearchCV(catboost_model_RGS,param_grid,cv=5,scoring='neg_mean_squared_error')
...
# Latih ulang model dengan hyperparameter terbaik pada seluruh data pelatihan
best_catboost_model_RGS = CatBoostRegressor(**best_params_RGS)
best_catboost_model_RGS.fit(X_RGS_train, y_RGS_train)

# Lakukan prediksi pada data pengujian
y_RGS_pred = best_catboost_model_RGS.predict(X_RGS_test)
...
plt.title("Scatter Plot Umur Bintang\n Menggunakan CatBoost") # Beri judul scatter plot.
...
# Gunakan fungsi open().
modelMSS = 'C:\\Users\\ACER\\Downloads\\Tugas Akhir Astronomi\\Uji Coba CatBoost\\Version 2\\modelMSS.txt'

# Buka file di write mode
with open(modelMSS,'w') as file:
  # Write content to the file
  file.write(str(best_catboost_model_MSS))

    
# Simpan model untuk Bintang Raksasa Merah.
# Gunakan fungsi open().
modelRGS = 'C:\\Users\\ACER\\Downloads\\Tugas Akhir Astronomi\\Uji Coba CatBoost\\Version 2\\modelRGS.txt'

# Buka file di write mode
with open(modelRGS,'w') as file:
  # Write content to the file
  file.write(str(best_catboost_model_RGS))
...
# Gunakan fungsi open().
bestparamsMSS = 'C:\\Users\\ACER\\Downloads\\Tugas Akhir Astronomi\\Uji Coba CatBoost\\Version 2\\bestparamsMSS.txt'
...
# Gunakan fungsi open().
bestparamsRGS = 'C:\\Users\\ACER\\Downloads\\Tugas Akhir Astronomi\\Uji Coba CatBoost\\Version 2\\bestparamsRGS.txt'
...
# Simpan data.
df_y_MSS_diff_table.to_csv('C:\\Users\\ACER\\Downloads\\Tugas Akhir Astronomi\\Uji Coba CatBoost\\Version 2\\MainSequenceStars.csv')
df_y_RGS_diff_table.to_csv('C:\\Users\\ACER\\Downloads\\Tugas Akhir Astronomi\\Uji Coba CatBoost\\Version 2\\RedGiantStars.csv')
...
# Simpan data.
df_metric_MSS.to_csv('C:\\Users\\ACER\\Downloads\\Tugas Akhir Astronomi\\Uji Coba CatBoost\\Version 2\\Metric_MainSequenceStars.csv')
df_metric_RGS.to_csv('C:\\Users\\ACER\\Downloads\\Tugas Akhir Astronomi\\Uji Coba CatBoost\\Version 2\\Metric_RedGiantStars.csv')
